...
void load(std::vector<enemy>&mobs, sf::Texture &enemy_texture);
...
int main()
{
	...
	load(mobs,enemy_texture);//wczytywanie przeciwnikow
	...
	while (oknoAplikacji.isOpen())
	{
		...
		mobs[0].Draw(oknoAplikacji);
		mobs[1].Draw(oknoAplikacji);
		mobs[2].Draw(oknoAplikacji);
...
void load(std::vector<enemy>&mobs, sf::Texture &enemy_texture)
{
	...
	while (!enemies.eof())
	{
		mobs.push_back(enemy(enemy_texture));
		temp_line.clear();
		...
		getline(enemies, temp_line);
		mobs[i].set_speed(std::stod(temp_line));
		i++;
	}
}